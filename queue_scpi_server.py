...
def start_cv_stream():
    ...
    try:
        ...
        # 🎯 CRITICAL: Send current range command FIRST to prevent overload
        current_range = parameters.get('current_range', 'auto')  # Default to auto range
        if current_range != 'auto':
            try:
                range_value = int(current_range)
                range_command = f"POTEn:CURRent:RANGe {range_value}"
                logger.info(f"📡 CV Sending current range command FIRST: {range_command}")
                range_result = scpi_server.send_command(range_command)
                if not range_result["success"]:
                    logger.warning(f"⚠️ CV Current range command failed: {range_result.get('error', 'Unknown error')}")
                else:
                    logger.info(f"✅ CV Current range set to {current_range}")
            except (ValueError, TypeError):
                logger.warning(f"⚠️ CV Invalid current_range value: {current_range}, using auto range")
        else:
            logger.info(f"🤖 CV Using AUTO range mode - STM32 will handle range selection automatically")
