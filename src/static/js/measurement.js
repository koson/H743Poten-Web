...
function startDataCollection() {
    const dataCollector = setInterval(async () => {
        ...
        try {
            ...
            if (data.error) {
                console.error('Data collection error:', data.error);
                clearInterval(dataCollector);
                return;
            }
            ...
            if (data.points && data.points.length > 0) {
                ...
                data.points.forEach(point => {
                    dataPoints.time.push(point.timestamp);
                    dataPoints.potential.push(point.voltage);
                    dataPoints.current.push(point.current);
                });
...
async function loadPorts() {
    try {
        const response = await fetch('/api/connection/ports');
        const data = await response.json();
