...
def find_stm32_port():
    ...
    # Check common STM32 ports (prioritize ACM1 as requested)
    possible_ports = [
        ...
        if os.path.exists(port):
            try:
                ...
                if 'MANUFACTURE' in response or 'INSTR' in response:
                    print(f"✅ Found STM32 at {port}: {response}")
                    return port
                    
            except Exception as e:
                continue
    
    print("⚠️ STM32 not found, using default /dev/ttyACM2")
    return '/dev/ttyACM2'

# Global debug settings
ENABLE_AUTO_RANGE_DEBUG = False  # เปิด/ปิดการแสดง AUTO-RANGE debug
...
def get_cv_parameters_from_env():
    """Get CV parameters from environment variables or use defaults"""
    try:
        start_v = float(os.environ.get('CV_START_V', '1.0'))
        end_v = float(os.environ.get('CV_END_V', '-1.0'))
        scan_rate_mv_s = float(os.environ.get('CV_SCAN_RATE', '100'))  # mV/s
        cycles = int(os.environ.get('CV_CYCLES', '1'))
        step_size = float(os.environ.get('CV_STEP_SIZE', '10'))  # mV
        sample_interval = float(os.environ.get('CV_SAMPLE_INTERVAL', '100'))  # ms
        ...
        print(f"⚠️ Error parsing environment parameters: {e}")
        return None
...
def test_cv_scan():
    ...
    try:
        ...
        # Get CV parameters from environment or use defaults
        env_params = get_cv_parameters_from_env()
        
        if env_params:
            # Use parameters from GUI
            test_cases = [
                {
                    'name': f'CV Test: {env_params["rate"]*1000:.0f} mV/s',
                    'begin': env_params['begin'],
                    'upper': env_params['upper'],
                    'lower': env_params['lower'],
                    'rate': env_params['rate'],
                    'cycles': env_params['cycles']
                }
            ]
            print(f"📊 Using GUI parameters:")
            print(f"   Voltage: {env_params['upper']} V → {env_params['lower']} V")
            print(f"   Scan rate: {env_params['rate']*1000:.0f} mV/s")
            print(f"   Cycles: {env_params['cycles']}")
        else:
...
if __name__ == "__main__":
    # Check if called from standalone app (has CV environment variables)
    if any(key.startswith('CV_') for key in os.environ.keys()):
        print("🎯 Running CV test with GUI parameters...")
        try:
            test_cv_scan()
        except KeyboardInterrupt:
